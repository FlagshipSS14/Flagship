...
using Content.FlagShip.Client.Radar;
using Content.FlagShip.Shared.Company;
using Content.FlagShip.Shared.Shuttles.Events;
...
namespace Content.FlagShip.Client.Shuttles.UI
{
    public partial class ShuttleNavControl // Mono
    {
        ...
        private static void NfAddBlipToList(List<BlipData> blipDataList, bool isOutsideRadarCircle, Vector2 uiPosition, int uiXCentre, int uiYCentre, Color color, EntityUid gridUid = default)
        {
            ...
            if (gridUid != default &&
                IoCManager.Resolve<IEntityManager>().TryGetComponent(gridUid, out Content.Shared._Mono.Company.CompanyComponent? companyComp) &&
