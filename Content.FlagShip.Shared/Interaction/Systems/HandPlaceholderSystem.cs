using Content.FlagShip.Shared.Interaction.Components;
...
namespace Content.FlagShip.Shared.Interaction.Systems;
...
public sealed partial class HandPlaceholderSystem : EntitySystem
{
    ...
    public readonly EntProtoId<Components.HandPlaceholderComponent> Placeholder = "HandPlaceholder";

    public override void Initialize()
    {
        SubscribeLocalEvent<Components.HandPlaceholderRemoveableComponent, EntGotRemovedFromContainerMessage>(OnEntityRemovedFromContainer);

        SubscribeLocalEvent<Components.HandPlaceholderComponent, BeforeRangedInteractEvent>(BeforeRangedInteract);
        SubscribeLocalEvent<Components.HandPlaceholderComponent, ContainerGettingRemovedAttemptEvent>(OnRemoveAttempt);
    }
    ...
    public void SpawnPlaceholder(BaseContainer container, EntityUid item, EntProtoId id, EntityWhitelist whitelist)
    {
        ...
        var comp = Comp<Components.HandPlaceholderComponent>(placeholder);
        comp.Prototype = id;
    ...
    {
        var comp = EnsureComp<Components.HandPlaceholderRemoveableComponent>(item);
    ...
    {
        if (TryComp<Components.HandPlaceholderRemoveableComponent>(item, out var comp))
        {
            comp.Enabled = enabled;
            Dirty(item, comp);
        }
        else if (TryComp<Components.HandPlaceholderComponent>(item, out var placeholder))
        {
    ...
    private void SwapPlaceholder(Entity<Components.HandPlaceholderRemoveableComponent> ent, BaseContainer container)
    {
        ...
        RemCompDeferred<Components.HandPlaceholderRemoveableComponent>(ent);
    ...
    private void OnEntityRemovedFromContainer(Entity<Components.HandPlaceholderRemoveableComponent> ent, ref EntGotRemovedFromContainerMessage args)
    {
        SwapPlaceholder(ent, args.Container);
    }

    private void BeforeRangedInteract(Entity<Components.HandPlaceholderComponent> ent, ref BeforeRangedInteractEvent args)
    {
    ...
    private void OnRemoveAttempt(Entity<Components.HandPlaceholderComponent> ent, ref ContainerGettingRemovedAttemptEvent args)
    {
        if (ent.Comp.Enabled)
            args.Cancel();
    }

    private void TryToPickUpTarget(Entity<Components.HandPlaceholderComponent> ent, EntityUid target, EntityUid user)
    {
