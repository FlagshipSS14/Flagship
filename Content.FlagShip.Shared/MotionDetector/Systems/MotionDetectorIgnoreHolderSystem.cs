using Content.FlagShip.Shared.MotionDetector.Components;
using Content.Shared.Hands.Components;
using Content.Shared.ProximityDetection;
using Robust.Shared.Containers;
using Robust.Shared.Physics.Components;

namespace Content.FlagShip.Shared.MotionDetector.Systems;
...
public sealed partial class MotionDetectorIgnoreHolderSystem : EntitySystem
{
    ...
    private void OnProximityDetectionAttempt(EntityUid targetEntity, MetaDataComponent component, ref ProximityDetectionAttemptEvent args)
    {
        ...
        if (!HasComp<Components.MotionDetectorIgnoreHolderComponent>(detectorEntity))
            return;
    ...
    {
        if (!TryComp<Company.CompanyComponent>(holder, out var holderCompany))
            return false;

        if (!TryComp<Company.CompanyComponent>(target, out var targetCompany))
            return false;
