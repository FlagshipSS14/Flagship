...
public sealed partial class ShipAccessReaderSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<FlagShip.Shared.Shipyard.ShipAccessReaderComponent, StorageOpenAttemptEvent>(OnStorageOpenAttempt);
        SubscribeLocalEvent<FlagShip.Shared.Shipyard.ShipAccessReaderComponent, BeforeDoorOpenedEvent>(OnBeforeDoorOpened);
        SubscribeLocalEvent<FlagShip.Shared.Shipyard.ShipAccessReaderComponent, LockToggleAttemptEvent>(OnLockToggleAttempt);
    }

    private void OnStorageOpenAttempt(EntityUid uid, FlagShip.Shared.Shipyard.ShipAccessReaderComponent component, ref StorageOpenAttemptEvent args)
    {
    ...
    private void OnBeforeDoorOpened(EntityUid uid, FlagShip.Shared.Shipyard.ShipAccessReaderComponent component, ref BeforeDoorOpenedEvent args)
    {
        if (!component.Enabled)
            return;
    ...
    private void OnLockToggleAttempt(EntityUid uid, FlagShip.Shared.Shipyard.ShipAccessReaderComponent component, ref LockToggleAttemptEvent args)
    {
        if (!component.Enabled)
            return;
    ...
    public bool HasShipAccess(EntityUid user, EntityUid target, FlagShip.Shared.Shipyard.ShipAccessReaderComponent component, bool silent = false)
    {
        ...
        if (!TryComp<Components.ShuttleDeedComponent>(gridUid, out var shipDeed))
        {
        ...
        if (TryComp<Company.CompanyComponent>(gridUid, out var shipCompany))
        {
        ...
        foreach (var cardUid in accessibleCards)
        {
            if (TryComp<Components.ShuttleDeedComponent>(cardUid, out var cardDeed))
            {
        ...
        {
            if (TryComp<Components.ShuttleDeedComponent>(voucherUid, out var voucherDeed))
            {
    ...
    private HashSet<EntityUid> FindAccessibleVouchers(EntityUid user)
    {
        ...
        foreach (var item in _handsSystem.EnumerateHeld(user))
        {
            ...
            if (HasComp<FlagShip.Shared.Whitelist.Components.NFShipyardVoucherComponent>(item))
                vouchers.Add(item);
        }
        ...
        {
            // Check if the item is a voucher
            if (HasComp<FlagShip.Shared.Whitelist.Components.NFShipyardVoucherComponent>(idUid.Value))
