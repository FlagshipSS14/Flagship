...
{
    public void UpdateDeclaw(EntityUid uid, ClawTypes.Declawed declawed, Components.ClawsComponent claws, float updateTime)
    {
    ...
    public void Declaw(EntityUid uid, Components.ClawsComponent claws)
    {
        ...
        if (!TryGetStage<ClawTypes.Declawed>(claws, out var declawedStage))
            return;

        _popup.PopupEntity(Robust.Shared.Localization.Loc.GetString("declaw-success"), uid, PopupType.LargeCaution);
    ...
    private void DeclawDrop(EntityUid uid, Hand hand, EntityUid? item)
    {
        ...
        _popup.PopupEntity(Robust.Shared.Localization.Loc.GetString("declaw-item-drop"), uid, PopupType.MediumCaution);
    }
