...
public abstract partial class SharedClawsSystem : EntitySystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<Components.ClawsComponent, GetMeleeDamageEvent>(OnMeleeAttack);
        SubscribeLocalEvent<Components.ClawsComponent, ShotAttemptedEvent>(TryShoot);
        SubscribeLocalEvent<Components.ClawsComponent, ExaminedEvent>(OnExamine);

        InitializeNailClippers();
    }

    private void OnMeleeAttack(Entity<Components.ClawsComponent> ent, ref GetMeleeDamageEvent args)
    {
        if (!TryGetStage<ClawTypes.SharpClaw>(ent, out var stage) ||
            stage.Damage == null)
            return;
    ...
    private void TryShoot(Entity<Components.ClawsComponent> ent, ref ShotAttemptedEvent args)
    {
        if (!TryGetStage<ClawTypes.SharpClaw>(ent.Comp, out var stage))
            return;
    ...
    private void OnExamine(Entity<Components.ClawsComponent> ent,ref ExaminedEvent args)
    {
    ...
    /// </summary>
    public void UpdateClaws(EntityUid uid, Components.ClawsComponent component)
    {
        if (!TryGetStage<ClawTypes.SharpClaw>(component, out var stage) ||
    ...
    public void GrowClaws(TimeSpan bonusGrowth, Components.ClawsComponent component)
    {
        component.AccumulatedBonusGrowth += bonusGrowth;
    }

    protected bool TryGetStage<T>(Components.ClawsComponent comp, [NotNullWhen(true)] out T? stage) where T : ClawTypes.ClawType
    {
    ...
    protected bool TryGetStage(Components.ClawsComponent comp, [NotNullWhen(true)] out ClawTypes.ClawType? stage)
    {
        if (!_protoMan.TryIndex(comp.ClawStage, out var clawProto))
        {
            stage = null;
    ...
    protected int TryGetStageNumber(Components.ClawsComponent comp)
    {
