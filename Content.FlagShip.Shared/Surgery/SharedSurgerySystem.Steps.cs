using System.Linq;
using Content.FlagShip.Shared.Body.Events;
using Content.FlagShip.Shared.Body.Organ;
using Content.FlagShip.Shared.Body.Part;
using Content.FlagShip.Shared.BodyEffects;
using Content.FlagShip.Shared.Surgery.Conditions;
using Content.FlagShip.Shared.Surgery.Effects.Step;
using Content.FlagShip.Shared.Surgery.Steps;
using Content.FlagShip.Shared.Surgery.Steps.Parts;
using Content.FlagShip.Shared.Surgery.Tools;
...
public abstract partial class SharedSurgerySystem
{
    ...
    private void OnToolCanPerform(Entity<SurgeryStepComponent> ent, ref SurgeryCanPerformStepEvent args)
    {
        ...
        if (_inventory.TryGetContainerSlotEnumerator(args.Body, out var containerSlotEnumerator, args.TargetSlots))
        {
            while (containerSlotEnumerator.MoveNext(out var containerSlot))
            {
                ...
                args.Popup = Robust.Shared.Localization.Loc.GetString("surgery-ui-window-steps-error-armor");
                return;
            }
        }
    ...
    private void OnSurgeryTargetStepChosen(Entity<SurgeryTargetComponent> ent, ref SurgeryStepChosenBuiMsg args)
    {
        ...
        var doAfter = new DoAfterArgs(Robust.Shared.GameObjects.EntityManager, user, TimeSpan.FromSeconds(duration), ev, body, part)
        {
            BreakOnMove = true,
        ...
        {
            var userName = Identity.Entity(user, Robust.Shared.GameObjects.EntityManager);
            var targetName = Identity.Entity(ent.Owner, Robust.Shared.GameObjects.EntityManager);

            var locName = $"surgery-popup-procedure-{args.Surgery}-step-{args.Step}";
            var locResult = Robust.Shared.Localization.Loc.GetString(locName,
                ("user", userName), ("target", targetName), ("part", part));

            if (locResult == locName)
                locResult = Robust.Shared.Localization.Loc.GetString($"surgery-popup-step-{args.Step}",
                    ("user", userName), ("target", targetName), ("part", part));
    ...
    private bool AnyHaveComp(List<EntityUid> tools, IComponent component, out EntityUid withComp, out float speed)
    {
        ...
        {
            if (Robust.Shared.GameObjects.EntityManager.TryGetComponent(tool, component.GetType(), out var found) && found is ISurgeryToolComponent toolComp)
            {
                withComp = tool;
