...
namespace Content.FlagShip.Server.Radiation;
...
public sealed partial class ChainRadiationSystem : EntitySystem
{
    ...
    {
        var query = EntityQueryEnumerator<FlagShip.Server.Radiation.ChainRadiationComponent, RadiationReceiverComponent, RadiationSourceComponent>();
        while (query.MoveNext(out var uid, out var chain, out var receiver, out var source))
        {
            ...
            if (source.Intensity > chain.ExplosionThreshold)
            {
                ...
                foreach (var other in _lookup.GetEntitiesInRange<FlagShip.Server.Radiation.ChainRadiationComponent>(coord, chain.ChainExplosionRadius))
                {
    ...
    private void Explode(Entity<FlagShip.Server.Radiation.ChainRadiationComponent> ent)
    {
